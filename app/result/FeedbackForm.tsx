...
interface FeedbackResponse { error?: string; details?: string[]; }
    ...
    <fieldset>
      ...
      <div className="mt-3 flex gap-1" role="group" aria-label={label}>
        {RATING_VALUES.map((rating) => {
          ...
          return (
            <button key={rating} type="button" className="rounded p-1 text-brand-muted transition-colors hover:text-brand-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-ink focus-visible:ring-offset-2 focus-visible:ring-offset-brand-surface" onClick={() => onChange(rating)} aria-label={`${rating} out of 5 stars`} aria-pressed={value === rating}>
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("/api/feedback", {
        ...
        body: JSON.stringify({ generationId, hairstyleId, overallRating, identityRating, realismRating, decisionConfidence, issues, comment }),
      });
        ...
        <fieldset>
          ...
          <div className="mt-3 grid grid-cols-3 gap-3">
            {DECISION_OPTIONS.map((option) => (
              <label key={option.value} className={cn("flex min-h-11 cursor-pointer items-center justify-center rounded border px-3 text-sm font-medium transition-colors", decisionConfidence === option.value ? "border-brand-ink bg-brand-ink text-brand-canvas" : "border-brand-border text-brand-muted hover:border-brand-ink/40 hover:text-brand-ink")}>
