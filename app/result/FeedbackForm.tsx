          ...
          return (
            ...
            >
              <Star
                className={cn(
                  "h-7 w-7",
                  selected && "fill-current text-brand-ink"
                )}
                aria-hidden="true"
              />
            </button>
...
const canSubmit =
  ...
  if (isSubmitted) {
    return (
      <section
        className="mx-auto mt-16 max-w-2xl rounded-editorial border border-brand-border bg-brand-surface px-6 py-10 text-center shadow-sm sm:px-10"
        aria-live="polite"
      >
        <Check className="mx-auto h-7 w-7 text-brand-ink" aria-hidden="true" />
        <h2 className="mt-4 text-2xl font-semibold text-brand-ink">
          {isSubmitted ? "Thank you for your feedback." : "No problem."}
        </h2>
        <p className="mt-3 leading-relaxed text-brand-muted">
          {isSubmitted
            ? "Your feedback helps us improve future hairstyle previews."
            : "You can continue exploring whenever you are ready."}
        </p>
      </section>
    );
  }
  ...
  return (
    <section
      className="mx-auto mt-16 max-w-2xl rounded-editorial border border-brand-border bg-brand-surface p-6 shadow-sm sm:p-10"
      aria-labelledby="feedback-heading"
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 id="feedback-heading" className="text-2xl font-semibold text-brand-ink">
        ...
        <button
          ...
          className="shrink-0 text-sm font-medium text-brand-muted transition-colors hover:text-brand-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-ink focus-visible:ring-offset-2 focus-visible:ring-offset-brand-surface"
        >
          Not now
        </button>
      </div>
      ...
      {!hairstyleId && (
        <p className="mt-6 rounded border border-brand-border bg-brand-canvas px-4 py-3 text-sm text-brand-muted" role="status">
          Feedback is available after you select a hairstyle.
        </p>
      )}
      ...
      <form className="mt-8 space-y-8" onSubmit={handleSubmit}>
        ...
        <RatingInput
          label="How much does this still look like you?"
...
/>
        ...
        <fieldset>
          <legend className="text-base font-medium text-brand-ink">
            Would you consider this hairstyle in real life?
          </legend>
          <div className="mt-3 grid grid-cols-3 gap-3">
            {DECISION_OPTIONS.map((option) => (
              <label
                key={option.value}
                className={cn(
                  "flex min-h-11 cursor-pointer items-center justify-center rounded border px-3 text-sm font-medium transition-colors focus-within:ring-2 focus-within:ring-brand-ink focus-within:ring-offset-2 focus-within:ring-offset-brand-surface",
                  decisionConfidence === option.value
                    ? "border-brand-ink bg-brand-ink text-brand-canvas"
                    : "border-brand-border text-brand-muted hover:border-brand-ink/40 hover:text-brand-ink"
                )}
              >
                <input
                  type="radio"
                  ...
                  value={option.value}
                  checked={decisionConfidence === option.value}
                  onChange={() => setDecisionConfidence(option.value)}
                  className="sr-only"
                />
                {option.label}
              </label>
          ...
          <fieldset>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              {ISSUE_OPTIONS.map((issue) => (
                <label
                  key={issue.value}
                  className="flex cursor-pointer items-start gap-3 rounded border border-brand-border px-3 py-3 text-sm text-brand-muted transition-colors hover:border-brand-ink/40 hover:text-brand-ink"
                >
                  <input
                    type="checkbox"
                    checked={issues.includes(issue.value)}
                    onChange={() => toggleIssue(issue.value)}
                    className="mt-0.5 h-4 w-4 accent-[#181816]"
                  />
        ...
        <div>
          <label htmlFor="feedback-comment" className="text-base font-medium text-brand-ink">
            Additional comments <span className="text-brand-muted">(optional)</span>
          </label>
          <textarea
            id="feedback-comment"
            value={comment}
            ...
            maxLength={500}
            rows={4}
            className="mt-3 w-full resize-y rounded border border-brand-border bg-brand-canvas px-3 py-3 text-brand-ink outline-none transition-colors placeholder:text-brand-muted focus:border-brand-ink focus:ring-1 focus:ring-brand-ink"
            placeholder="Tell us anything that would make this preview more useful."
            aria-describedby="feedback-comment-count"
          />
          <p id="feedback-comment-count" className="mt-2 text-right text-xs text-brand-muted">
            {comment.length}/500
          </p>
        </div>

        {error && (
          <p className="rounded border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-700" role="alert">
            {error}
          </p>
        )}
