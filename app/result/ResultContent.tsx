    ...
    <main className="min-h-screen bg-brand-canvas px-6 py-20">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center mb-12">
          ...
          <h1 className="text-5xl md:text-6xl font-semibold tracking-tight text-brand-ink mb-5">Your Preview is Ready</h1>
          <p className="text-lg text-brand-muted max-w-2xl mx-auto leading-relaxed">Here&apos;s how <strong>{formattedStyle}</strong> could look on you.</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.25, ease: [0.16, 1, 0.3, 1] }} className="max-w-2xl mx-auto rounded-editorial overflow-hidden border border-brand-border bg-brand-surface shadow-editorial">
          <div className="aspect-[3/4] bg-brand-border/15">
            {generation.imageUrl ? (
              <img src={generation.imageUrl} alt={formattedStyle} className="w-full h-full object-cover" />
            ) : (
              <div className="flex h-full min-h-[720px] items-center justify-center px-10 text-center">
                <div>
                  <h2 className="text-3xl font-semibold text-brand-ink mb-4">{formattedStyle}</h2>
                  <p className="text-brand-muted leading-relaxed">No generated preview is available.</p>
                </div>
              </div>
            )}
          </div>
        ...
        <FeedbackForm
          ...
          existingFeedback={generation.existingFeedback}
        />
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 1.35 }} className="mt-10 flex flex-col sm:flex-row justify-center gap-8">
          ...
          <Link href="/upload" className="inline-flex items-center justify-center text-brand-muted hover:text-brand-ink transition-colors duration-300">
