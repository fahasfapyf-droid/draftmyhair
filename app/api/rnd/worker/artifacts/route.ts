...
export async function POST(request: Request) {
  ...
  if (!workerId || !jobId || !Number.isInteger(attemptNumber) || attemptNumber < 1 || !(file instanceof File)) {
    return NextResponse.json({ error: "workerId, jobId, attemptNumber and file are required" }, { status: 400 });
  }

  if (!file.type.startsWith("image/")) {
    return NextResponse.json({ error: "Artifact must be an image" }, { status: 415 });
  }

  if (file.size <= 0 || file.size > MAX_ARTIFACT_BYTES) {
    return NextResponse.json({ error: "Artifact size is outside the allowed range" }, { status: 413 });
  }

  const now = new Date();
  const job = await prisma.rnDJob.findUnique({
    where: { id: jobId },
    select: { id: true, leaseOwner: true, leaseExpiresAt: true, status: true },
  });

  if (!job) return NextResponse.json({ error: "Job not found" }, { status: 404 });
  if (job.status !== "PROCESSING" || job.leaseOwner !== workerId || (job.leaseExpiresAt && job.leaseExpiresAt < now)) {
    return NextResponse.json({ error: "Job lease is no longer valid" }, { status: 409 });
  }

  const buffer = Buffer.from(await file.arrayBuffer());
  const checksum = createHash("sha256").update(buffer).digest("hex");
  const storageKey = `rnd/attempts/${jobId}/${attemptNumber}-${checksum}.${file.type === "image/jpeg" ? "jpg" : file.type === "image/webp" ? "webp" : "png"}`;
