...
export async function POST(request: Request) {
  ...
  if (styles.length !== styleIds.length || sources.length !== sourceAssetIds.length) return NextResponse.json({ error: "One or more selected styles or source photos no longer exist." }, { status: 400 });
