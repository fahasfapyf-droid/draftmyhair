...
export async function POST(request: Request) {
  ...
  const qaStatus = typeof body.qaStatus === "string" ? body.qaStatus : "DRAFT";
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (requestedStatus === "ACTIVE") {
        await tx.promptVersion.updateMany({ where: { hairstyleId, status: "ACTIVE" }, data: { status: "ARCHIVED" } });
      }

      return tx.promptVersion.create({
        data: { hairstyleId, version, prompt, qaStatus, notes, status: requestedStatus },
