...
type Promo = { id: string; code: string; credits: number; maxRedemptions: number | null; redeemedCount: number; expiresAt: Date | null; isActive: boolean };

export function PromoCodeAdminPanel({ promos }: { promos: Promo[] }) {
  const [message, setMessage] = useState(""); const [busy, setBusy] = useState(false);
  async function create(formData: FormData) { setBusy(true); setMessage(""); const result = await createPromoCodeAction(formData); setMessage(result.message ?? ""); if (result.ok) window.location.reload(); setBusy(false); }
  async function toggle(id: string, active: boolean) { const fd = new FormData(); fd.set("id", id); fd.set("active", String(!active)); const result = await togglePromoCodeAction(fd); if (!result.ok) setMessage(result.message ?? "Unable to update promo code."); else window.location.reload(); }
  return <div className="space-y-6">
    <form action={create} className="rounded-editorial border border-brand-border bg-brand-surface p-6 shadow-sm">
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">
        <label className="text-sm">Code<input name="code" required maxLength={64} placeholder="QA10" className="mt-1 w-full rounded-lg border border-brand-border px-3 py-2" /></label>
