...
export async function createPromoCodeAction(formData: FormData) {
  ...
  try {
    ...
    await createPromoCode({ code: String(formData.get("code") ?? ""), credits: Number(formData.get("credits")), maxRedemptions: String(formData.get("maxRedemptions") ?? "").trim() ? Number(formData.get("maxRedemptions")) : null, startsAt: starts ? new Date(starts) : null, expiresAt: expires ? new Date(expires) : null });
    revalidatePath("/dashboard/admin/promo-codes");
    return { ok: true, message: "Promo code created." };
  } catch (error) { return { ok: false, message: error instanceof Error ? error.message : "Unable to create promo code." }; }
}
...
export async function togglePromoCodeAction(formData: FormData) {
  ...
  try {
    ...
    revalidatePath("/dashboard/admin/promo-codes");
    return { ok: true };
  } catch (error) { return { ok: false, message: error instanceof Error ? error.message : "Unable to update promo code." }; }
}
