...
export async function redeemPromoCodeAction(formData: FormData) {
  ...
  if (!session?.user?.id || session.user.role !== "USER") return { ok: false, message: "Please sign in as a customer to redeem a promo code." };
  try {
  ...
  } catch (error) { return { ok: false, message: error instanceof Error ? error.message : "Unable to redeem promo code." }; }
}
