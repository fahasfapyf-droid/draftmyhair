          ...
          {generations.length === 0 ? (
            ...
            <div className="mt-6 space-y-4">
              {generations.map((generation) => (
                <Link key={generation.id} href={`/dashboard/generations/${generation.id}`} className="block rounded border border-brand-border p-4 transition-colors hover:border-brand-ink">
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div className="relative h-16 w-16 overflow-hidden rounded border border-brand-border bg-brand-canvas">
                        {generation.outputImageUrl ? <img src={generation.outputImageUrl} alt={generation.hairstyle.name} className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-xs text-brand-muted">Pending</div>}
                      </div>
                      <div>
                        ...
                        {generation.userFeedback ? <div className="mt-2"><RatingStars rating={generation.userFeedback.overallRating} /></div> : <p className="mt-2 text-xs text-brand-muted">Not rated yet</p>}
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm text-brand-muted">{generation.createdAt.toLocaleDateString()}</p>
                      <p className="mt-1 text-sm font-medium text-brand-ink">View →</p>
                    </div>
                  </div>
                </Link>
              ))}
