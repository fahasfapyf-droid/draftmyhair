...
import { StylePhotoManager } from "./StylePhotoManager";
...
type Prompt = {
  ...
  qaStatus: string;
  notes: string | null;
  sourceRnDAttempt?: { id: string; attemptNumber: number; overallScore: number | string | null; artifactId: string | null } | null;
...
type Tab = "content" | "prompts" | "gallery" | "stylePhotos";
...
const GALLERY_CATEGORY_GROUPS = [
  { label: "Hairstyles", options: ["bob", "lob", "pixie", "bixie", "layers", "shag", "wolf", "mullet", "fade", "taper", "undercut", "crop", "crew", "quiff", "pompadour", "side-part", "comb-over", "mohawk", "man-bun", "braids", "locs", "afro", "curly", "bangs", "updo"] },
  { label: "Hair Colour", options: ["hair-colour"] },
  { label: "Buzz Cut", options: ["buzz-cut"] },
  { label: "Bald", options: ["bald"] },
  { label: "Beard", options: ["beard"] },
  { label: "Beard Removal", options: ["beard-removal"] },
] as const;
const GALLERY_CATEGORY_OPTIONS = GALLERY_CATEGORY_GROUPS.flatMap((group) => group.options);
const labelForGalleryCategory = (value: string) => value.replace(/-/g, " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
...
export function AdminContentLibrary() {
  ...
  const deleteContent = async (item: ContentItem) => {
    if (!window.confirm(`Delete ${item.name}? This permanently removes the content item and its database prompt versions. Customer generation history will block deletion.`)) return;
    setError(""); setNotice("");
    try {
      await json(`/api/dashboard/admin/content/styles/${item.id}`, { method: "DELETE" });
      await loadAll();
      setNotice(`${item.name} deleted.`);
    } catch (e) { setError(e instanceof Error ? e.message : "Unable to delete content item."); }
  };
  ...
  const deletePrompt = async (prompt: Prompt) => {
    if (prompt.id.startsWith("compiled:")) return;
    if (!window.confirm(`Delete ${prompt.hairstyle.name} v${prompt.version}? If this is the active database version, the engine will fall back to the compiled production prompt.`)) return;
    setError(""); setNotice("");
    try {
      await json(`/api/dashboard/admin/content/prompts/${prompt.id}`, { method: "DELETE" });
      await loadAll();
      setNotice(`${prompt.hairstyle.name} v${prompt.version} deleted.`);
    } catch (e) { setError(e instanceof Error ? e.message : "Unable to delete prompt version."); }
  };
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-wrap gap-2">{([['content','Content'],['prompts','Prompts'],['gallery','Gallery'],['stylePhotos','Style Photos']] as const).map(([key,label]) => <button key={key} onClick={() => setTab(key)} className={`rounded-lg px-4 py-2 text-sm font-semibold ${tab === key ? "bg-brand-ink text-white" : "border border-brand-border bg-brand-surface text-brand-ink"}`}>{label}</button>)}</div>
