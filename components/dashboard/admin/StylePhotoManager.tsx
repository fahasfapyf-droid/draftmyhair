...
export function StylePhotoManager() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : "Unable to load style photos.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const categoryOptions = useMemo(
    () => Array.from(new Set(serviceStyles.filter((style) => genderFilter === "ALL" || style.gender === genderFilter).map((style) => style.category).filter(Boolean) as string[])).sort(),
    [serviceStyles, genderFilter]
  );
  const visibleStyles = useMemo(
    () => serviceStyles.filter((style) => (genderFilter === "ALL" || style.gender === genderFilter) && (categoryFilter === "ALL" || style.category === categoryFilter)),
    [serviceStyles, genderFilter, categoryFilter]
  );
  const groupedStyles = useMemo(() => {
    const groups = new Map<string, StyleItem[]>();
    for (const style of visibleStyles) {
      const key = style.category || "UNCATEGORIZED";
      groups.set(key, [...(groups.get(key) || []), style]);
    }
