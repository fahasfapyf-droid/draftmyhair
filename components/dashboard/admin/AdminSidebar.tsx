...
const navigation = [
  ...
  { href: "/dashboard/admin/master-prompt", label: "Master Prompt" },
        ...
        {navigation.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center justify-between gap-3 rounded-lg px-3 py-2 text-sm transition-colors ${active ? "bg-brand-ink text-white" : "text-brand-muted hover:bg-brand-background hover:text-brand-ink"}`}
            >
              <span>{item.label}</span>
              {showUnreadMessages ? (
                <span aria-label={`${unreadMessageCount} unread customer message${unreadMessageCount === 1 ? "" : "s"}`} className="inline-flex min-w-5 items-center justify-center rounded-full bg-red-600 px-1.5 py-0.5 text-[11px] font-semibold leading-none text-white">
                  {unreadMessageCount > 99 ? "99+" : unreadMessageCount}
                </span>
              ) : null}
              {showUnreadFeedback ? (
                <span aria-label={`${unreadFeedbackCount} new customer feedback${unreadFeedbackCount === 1 ? "" : "s"}`} className="inline-flex min-w-5 items-center justify-center rounded-full bg-red-600 px-1.5 py-0.5 text-[11px] font-semibold leading-none text-white">
