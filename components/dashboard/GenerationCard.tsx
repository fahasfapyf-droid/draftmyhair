...
export function GenerationCard({
  ...
  async function handleDelete() {
    ...
    const confirmed = window.confirm(
      "Delete this hairstyle preview? This action cannot be undone."
    );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          data?.error ?? "Unable to delete generation."
        );
      }
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex flex-col gap-2">
          <Link
            ...
            className="rounded border border-brand-border px-3 py-2 text-center text-sm transition-colors hover:bg-brand-canvas"
          >
            View
          </Link>
          ...
          <button
            ...
            disabled={isDeleting}
            className="rounded border border-red-300 px-3 py-2 text-sm text-red-600 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
          >
