...
const decisionConfidenceValues = new Set<DecisionConfidence>(
  Object.values(DecisionConfidence)
);

const feedbackIssueValues = new Set<FeedbackIssue>(
  Object.values(FeedbackIssue)
);
...
export function validateFeedbackInput(
  ...
  if (typeof hairstyleId !== "string" || hairstyleId.trim().length === 0) {
    errors.push("hairstyleId is required.");
  }

  if (!isRating(overallRating)) {
...
if (!isRating(realismRating)) {
  ...
  if (
    ...
    !decisionConfidenceValues.has(decisionConfidence as DecisionConfidence)
  ) {
  ...
  if (
    ...
    (!Array.isArray(issues) ||
      !issues.every(
        (issue) =>
          typeof issue === "string" &&
          feedbackIssueValues.has(issue as FeedbackIssue)
      ))
  ) {
  ...
  if (typeof comment === "string" && comment.length > 500) {
    errors.push("comment must be 500 characters or fewer.");
  }
