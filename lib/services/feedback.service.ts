...
export function validateFeedbackInput(value: unknown): FeedbackValidationResult {
  ...
  if (typeof decisionConfidence !== "string" || !decisionConfidenceValues.has(decisionConfidence as DecisionConfidence)) errors.push("decisionConfidence is invalid.");
  if (issues !== undefined && (!Array.isArray(issues) || !issues.every((issue) => typeof issue === "string" && feedbackIssueValues.has(issue as FeedbackIssue)))) errors.push("issues must contain valid feedback issue values.");
