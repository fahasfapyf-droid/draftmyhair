...
function isRetryableGenerationError(error: unknown) {
  ...
  const retryablePatterns = ["timeout", "timed out", "deadline", "429", "500", "502", "503", "504", "network", "connection", "unavailable", "temporarily", "internal server error"];
...
export async function generatePreview(context: GenerationContext): Promise<GenerationResult> {
  try {
    ...
    if (!providerResult.provider || !providerResult.providerModel) return { success: false, error: "Provider returned incomplete metadata." };
